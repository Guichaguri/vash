...
//! "eventually" is the problem: a KCP frame needs a known opcode in byte 0 and
...
use cache_proto::kcp::{Opcode, encode_request, encode_set_body, encode_tag_sync_body};
...
fn kcp_seeds(root: &Path) {
    let dir = root.join("kcp_decode");
    ...
    cache_proto::kcp::encode_touch_body(&mut touch, b"some:key", 300);
    ...
    cache_proto::kcp::encode_key_list_body(&mut keys, &[b"a".as_slice(), b"bb", b"ccc"]);
    write(&dir, "get_many", &frame(Opcode::GetMany, &keys));
    write(&dir, "delete_many", &frame(Opcode::DeleteMany, &keys));

    let mut batch = Vec::new();
    cache_proto::kcp::encode_batch_count(&mut batch, 2);
...
fn record_seeds(root: &Path) {
    use cache_core::{RecordMeta, TagRef, encode_record};
    ...
    truncated.truncate(cache_core::RECORD_HEADER_LEN + 4);

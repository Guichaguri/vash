...
proptest! {
    #[test]
    fn kcp_decoding_arbitrary_bytes_never_panics(raw in proptest::collection::vec(any::<u8>(), 0..512)) {
        let _ = kcp::decode(&raw);
        let _ = peek_frame_len(&raw);
    }

    #[test]
    fn kcp_decoding_a_plausible_frame_never_panics(raw in plausible_frame()) {
        let _ = kcp::decode(&raw);
    }
    ...
    #[test]
    fn a_rejected_kcp_frame_reports_a_length_that_makes_progress(raw in plausible_frame()) {
        if let Err(DecodeError::Body { consumed, .. }) = kcp::decode(&raw) {
    ...
    fn peeking_and_decoding_agree_on_the_frame_boundary(raw in plausible_frame()) {
        match (peek_frame_len(&raw), kcp::decode(&raw)) {
    ...
    fn pipelined_frames_decode_one_after_another(
        ...
        for (index, key) in keys.iter().enumerate() {
            let Ok(Decoded::Request { request, consumed }) = kcp::decode(rest) else {
                prop_assert!(false, "frame {index} did not decode");
                unreachable!()
            };
            prop_assert_eq!(request.request_id, index as u32);
            match request.command {
                cache_core::Command::Get { key: decoded } => {
    ...
    fn a_set_body_survives_encoding_and_decoding(
        ...
        kcp::encode_set_body(&mut body, &key, &value, ttl_secs, &tag_refs);

        let mut frame = Vec::new();
        encode_request(&mut frame, Opcode::Set, 1, &body);

        let Ok(Decoded::Request { request, .. }) = kcp::decode(&frame) else {
    ...
    fn a_tag_sync_body_survives_encoding_and_decoding(
        ...
        let mut body = Vec::new();
        kcp::encode_tag_sync_body(&mut body, full, offered.iter().copied());

        let (decoded_full, decoded) = kcp::decode_tag_sync(&body).expect("what we encoded");
    ...
    fn a_storage_command_is_framed_by_its_length_not_its_content(
        ...
        let cache_core::Command::Set(set) = parsed.command else {
            prop_assert!(false, "expected a set");
            unreachable!()
        };
        prop_assert_eq!(set.value, value.as_slice());

...
//!
//! ```text
//! cargo run --release -p cache-bench --bin load -- --workload get --connections 64
//! cargo run --release -p cache-bench --bin load -- --addr 10.0.0.5:11311 --workload set
//! ```
//!
//! Drives real KCP frames over a real socket, because that is what a client
...
//! That is what the KCP frame format exists for — `request_id` is echoed so a
...
use cache_proto::kcp::{FrameLen, Opcode, encode_request, encode_set_body};
...
impl Inbound {
    ...
    async fn next_frame(&mut self, stream: &mut TcpStream) -> std::io::Result<&[u8]> {
        loop {
            match cache_proto::kcp::peek_frame_len(self.pending()) {
...
async fn main() -> anyhow::Result<()> {
    ...
    let (addr, _server) = match &options.addr {
        ...
        None => {
            ...
            let mut config = cache_server::Config::default();
            ...
            let server = cache_server::Server::bind(config).await?;

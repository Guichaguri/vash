...
use cache_core::{ClusterInfo, ClusterMode, MAX_TAG_SYNC_ENTRIES, PeerInfo, TagGeneration};
...
enum PeerError {
    ...
    ConnectTimeout,
    #[error("{0}")]
    Client(#[from] cache_client::ClientError),
...
///
/// One connection rather than one per message: the KCP client is
...
async fn peer_loop(
    ...
    let mut client: Option<cache_client::Client> = None;
...
async fn exchange(
    client: &mut Option<cache_client::Client>,
    peer: &Peer,
    timeout: Duration,
    full: bool,
    entries: &[(&[u8], u64)],
...
async fn try_exchange(
    ...
    for attempt in 0..2 {
        if client.is_none() {
            *client = Some(
                tokio::time::timeout(timeout, cache_client::Client::connect(&peer.addr))
                    .await
...
fn build_digest(
    store: &LmdbStore,
    offset: usize,
) -> cache_store::Result<(bool, Vec<TagGeneration>)> {

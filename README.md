...
| TLS | Working — TLS 1.3 on a second port, including cluster peers, **off unless enabled**, and needs `--features tls`. `redis-cli --tls` and memcached TLS clients work unchanged |
| mTLS | Working — a client certificate satisfies `auth.required` on its own, matched by Subject Alternative Name against an `mtls:` row in the credential file |
...
clients that wait for their answers, vash leads both on reads — 165,672 `GET`s a
second against Redis's 53,310 and memcached's 140,011 — and trails Redis on
writes, 44,930 `SET`s against 60,810. With clients that pipeline, reads lead
again (718,449 against 482,181 and 707,270) and writes are three to five times
behind.

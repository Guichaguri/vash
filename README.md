...
**Status: M0 complete.** `GET`/`SET`/`DELETE` over the native binary protocol
(KCP), with TTLs already honoured on the read path. Not yet production-usable —
...
| | Status |
|---|---|
| `HELLO`, `PING`, `GET`, `SET`, `DELETE` over KCP | Working |
| TTLs | Stored, and enforced on read. Expired records are never served. |
| Expiry sweeper (reclaiming the disk space) | M1 |
| Batch operations, group commit | M1 |
| Tags and `DELETE_BY_TAG` | M2 — currently **rejected** with `UNSUPPORTED` rather than silently ignored |
| Memcached protocol | M3 |
| Sharding, capacity eviction, metrics | M4 |

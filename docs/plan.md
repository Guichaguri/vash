...
| **M12** | TLS: `rustls` behind a feature flag, termination on a second listener, the connection loop generic over its stream, `ssl_enabled` and `vash_tls` per connection, and cluster peers dialled over TLS — proposed and measured in [tls-proposal.md](tls-proposal.md) | Phase 0's numbers published and its wrong answers corrected; a 1 MiB pipelined batch completes over TLS in both directions; both ports serve one store; a `tls.listen` in a build without the feature refuses to start; `redis-cli --tls` and a real memcached client drive it unchanged; a three-node cluster converges over TLS including a peer that was down |
